module.exports = function gruntWebpackTask(grunt) {
  ...
  grunt.registerMultiTask('webpack', 'Webpack files.', function() {
    ...
    if (!targetCompilers.hasOwnProperty(target)) {
      targetCompilers[target] = configureCompiler({
        name: this.name,
        target: target,
      });
    }

    runCompiler(targetCompilers[target]);
  });

  function configureCompiler(scope) {
    var name = scope.name,
        target = scope.target;
    ...
    var watch = some(options, 'watch');
    var cache = some(options, 'cache') && !watch;
    if (cache) {
